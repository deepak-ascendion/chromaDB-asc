...
use crate::types::{
    ...
    SpannerRowRef, SpannerRows, SysDbError, UpdateCollectionCursor, UpdateCollectionRequest, UpdateCollectionResponse,
};
use chroma_types::{
...
mod tests {
    ...
    async fn test_k8s_mcmr_integration_update_collection_increment_compaction_failure_count() {
        ...
        let collection = fetch_collection(&backend, collection_id, &tenant_id, &db_name).await;
        assert_eq!(collection.compaction_failure_count, Some(0));
        ...
        let collection = fetch_collection(&backend, collection_id, &tenant_id, &db_name).await;
        assert_eq!(collection.compaction_failure_count, Some(1));
    }
    ...
    async fn test_k8s_mcmr_integration_update_collection_increment_compaction_failure_count_multiple_times() {
        ...
        for i in 1..=3 {
            ...
            // Verify the count
            let collection = fetch_collection(&backend, collection_id, &tenant_id, &db_name).await;
            assert_eq!(collection.compaction_failure_count, Some(i));
        }
    }
    ...
    async fn test_k8s_mcmr_integration_update_collection_increment_compaction_failure_count_local_region() {
        ...
        let collection = fetch_collection(&backend, collection_id, &tenant_id, &db_name).await;
        assert_eq!(collection.compaction_failure_count, Some(1)); // Local region should have count 1
    }

    #[tokio::test]
    async fn test_k8s_mcmr_integration_update_collection_increment_compaction_failure_count_nonexistent_cursor() {
        let Some(backend) = setup_test_backend().await else {
            panic!("Skipping test: Spanner emulator not reachable. Is Tilt running?");
        };
        ...
        let name = format!("test_coll_no_cursor_{}", Uuid::new_v4());

        // Create collection with HNSW schema (no compaction cursors)
        ...
        // Try to increment compaction failure count for non-existent cursor
        let update_req = UpdateCollectionRequest {
            database_name: db_name.clone(),
            id: collection_id,
            name: None,
        ...
        let result = backend.update_collection(update_req).await;
        assert!(result.is_err(), "Should fail for non-existent compaction cursor");

        match result.unwrap_err() {
            SysDbError::NotFound(msg) => {
                assert!(
                    msg.contains("collection_compaction_cursor") && msg.contains("not found"),
                    "Expected NotFound error for missing cursor, got: {}",
                    msg
                );
            }
            e => panic!("Expected NotFound error, got: {:?}", e),
