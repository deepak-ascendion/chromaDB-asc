use crate::types::{self as internal, validate_uuid, CollectionFilter};
...
impl SysDb for SysdbService {
    ...
    async fn flush_collection_compaction(
        ...
        // Create a basic request to get the collection
        let get_collection_req = internal::GetCollectionsRequest {
            filter: CollectionFilter::default().ids(vec![collection_id]),
        };

        // Get the collection to determine the new version
        let backend = get_collection_req.assign(&self.backends);
        let collection_response = get_collection_req
            .run(backend)
            .await
            .map_err(|e: SysDbError| Status::from(e))?;

        let collection = collection_response.collections.first().ok_or_else(|| {
        ...
        let internal_req = FlushCompactionRequest {
            collection_id,
            tenant_id: proto_req.tenant_id,
            log_position: proto_req.log_position,
            current_collection_version: proto_req.collection_version,
            flush_segment_compaction_infos: proto_req.segment_compaction_info,
            ...
            schema_str: proto_req.schema_str,
            new_version_file,
            version_file_path,
            new_version,
        };
        ...
        let proto_resp: FlushCollectionCompactionResponse = internal_resp
            .try_into()
            .map_err(|e: SysDbError| Status::from(e))?;
    ...
    async fn create_new_version_file(
        ...
        let mut version_file_pb = match &collection.version_file_path {
            Some(path) => {
                ...
                version_file_manager.fetch_version_file(path).await?
            }
            None => {
                // Create new version file
                chroma_types::chroma_proto::CollectionVersionFile::default()
            }
        };

        // Add new version to the version file
...
mod tests {
    ...
    fn create_test_segment_compaction_info(
    ...
    async fn test_k8s_mcmr_integration_flush_collection_compaction() {
        ...
        let segment_compaction_info = create_test_segment_compaction_info(collection_id);
        let segment_uuid =
        ...
        assert!(version_file_path.contains(&format!("/{}", collection.version)));
    ...
    async fn test_k8s_mcmr_integration_flush_collection_compaction_invalid_collection_id() {
        ...
        let segment_compaction_info =
            create_test_segment_compaction_info(CollectionUuid(Uuid::new_v4()));
        let proto_req = FlushCollectionCompactionRequest {
            tenant_id: "test_tenant".to_string(),
            collection_id: invalid_collection_id.to_string(),
            segment_compaction_info: segment_compaction_info,
    ...
    async fn test_k8s_mcmr_integration_flush_collection_compaction_cross_region_version_consistency(
        ...
        // US and EU should have the same version file paths (shared database)
        assert_eq!(
            us_version_file_path_final, eu_version_file_path_final,
            "US and EU should have the same version file paths (shared database)"
        );

        // Verify the version file path formats are correct
        assert!(us_version_file_path_final.contains("versionfiles/"));
        assert!(us_version_file_path_final.contains(&format!("/{}", us_version_final)));
        assert!(us_version_file_path_final.contains("_flush"));

        assert!(eu_version_file_path_final.contains("versionfiles/"));
        assert!(eu_version_file_path_final.contains(&format!("/{}", eu_version_final)));
